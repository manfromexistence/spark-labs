  ...
  return (
    <>
      <main className="w-full py-5 px-[5%] h-auto mb-10 min-h-[90vh]">
        <div className="flex items-center justify-between mb-6">
          <span className="text-center font-display text-lg font-bold tracking-[-0.02em] drop-shadow-sm md:text-3xl md:leading-[5rem]">{
            // users.map((user:any) => user.userId === params.slug && user.username.toUpperCase()) || 
            "Student"}'s Submission!</span>
        ...
        <div className="admin-panel-lists">
          {submissions.map((items: any) => {
            return items.userId === params.slug ? (<div key={items.id}>
